...
BEGIN
        DRAWBAR(MEMBER_REC);
        ASSIGN(SCREEN_FILE,'MEMBER.SCN'); REWRITE(SCREEN_FILE,1);
  BLOCKWRITE(SCREEN_FILE,SCREEN^,4000);
  CLOSE(SCREEN_FILE);
        REPEAT
            D:=READKEY;
            CHOICE:=CODES(D);
...
BEGIN
        DRAWBAR(BOOK_REC);
        ASSIGN(SCREEN_FILE,'BOOK.SCN'); REWRITE(SCREEN_FILE,1);
  BLOCKWRITE(SCREEN_FILE,SCREEN^,4000);
  CLOSE(SCREEN_FILE);
        REPEAT
            D:=READKEY;
...
BEGIN
        DRAWBAR(ISSUE_REC);
        ASSIGN(SCREEN_FILE,'ISSUE.SCN'); REWRITE(SCREEN_FILE,1);
  BLOCKWRITE(SCREEN_FILE,SCREEN^,4000);
  CLOSE(SCREEN_FILE);
        REPEAT
            D:=READKEY;
...
BEGIN
        DRAWBAR(VIEW_REC);
        ASSIGN(SCREEN_FILE,'VIEW.SCN'); REWRITE(SCREEN_FILE,1);
  BLOCKWRITE(SCREEN_FILE,SCREEN^,4000);
  CLOSE(SCREEN_FILE);
        REPEAT
            D:=READKEY;
...
BEGIN
        DRAWBAR(SEARCH_REC);
        ASSIGN(SCREEN_FILE,'SEARCH.SCN'); REWRITE(SCREEN_FILE,1);
  BLOCKWRITE(SCREEN_FILE,SCREEN^,4000);
  CLOSE(SCREEN_FILE);
        REPEAT
            D:=READKEY;
...
BEGIN
        DRAWBAR(SORT_REC);
        ASSIGN(SCREEN_FILE,'SORT.SCN'); REWRITE(SCREEN_FILE,1);
  BLOCKWRITE(SCREEN_FILE,SCREEN^,4000);
  CLOSE(SCREEN_FILE);
        REPEAT
            D:=READKEY;
...
BEGIN
        DRAWBAR(OTHER_REC);
        ASSIGN(SCREEN_FILE,'OTHER.SCN'); REWRITE(SCREEN_FILE,1);
  BLOCKWRITE(SCREEN_FILE,SCREEN^,4000);
  CLOSE(SCREEN_FILE);
        REPEAT
            D:=READKEY;
    ...
    CASE CHOICE OF
     ...
     LEFT   :BEGIN ERASE_OLD(COPTION);
              IF COPTION=1 THEN COPTION:=MAX_OPTIONS ELSE COPTION:=COPTION-1;
               SHOW_NEW(COPTION);
             END;
     ENTER  :{EXIT TO OPTIONS};
     ESC    :COPTION:=MAX_OPTIONS+1;
     END;
     UNTIL (CHOICE=ENTER) OR (CHOICE=ESC);
END;
